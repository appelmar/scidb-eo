...
R CMD Rserve
git clone https://github.com/mappl/r_exec.git 
cd r_exec 
...
#********************************************************
/opt/scidb/15.7/bin/iquery -aq "load_library('r_exec')"
/opt/scidb/15.7/bin/iquery -aq "r_exec(build(<z:double>[i=1:100,10,0],0),'expr=x<-runif(1000);y<-runif(1000);list(sum(x^2+y^2<1)/250)')"
EOF
