#!/bin/bash
export CONTAINER_NAME=scidb-gis-1507
export IMAGE_NAME=${CONTAINER_NAME}_img
export HOST_PORT_SSH=35350
...
docker run -d --name="${CONTAINER_NAME}" --ipc="host"  -h "${CONTAINER_NAME}"  -p ${HOST_PORT_SSH}:22 -p ${HOST_PORT_SHIM}:8083 -p ${HOST_PORT_SCIDB}:1239 -v ${HOST_SHAREDFOLDER}:/opt/shared  ${IMAGE_NAME}
